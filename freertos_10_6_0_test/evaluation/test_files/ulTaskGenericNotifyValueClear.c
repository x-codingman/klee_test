...
klee_make_symbolic_controllable(&KLEE_ulBitsToClear, sizeof(KLEE_ulBitsToClear), "KLEE_ulBitsToClear", true);
if (KLEE_xTask < KERNEL_REGION_START || KLEE_xTask > KERNEL_REGION_END) { return 0; }
ulTaskGenericNotifyValueClear(KLEE_xTask,KLEE_uxIndexToClear,KLEE_ulBitsToClear);
return 0;
}
void vTaskFunction1( void *pvParameters )
{
