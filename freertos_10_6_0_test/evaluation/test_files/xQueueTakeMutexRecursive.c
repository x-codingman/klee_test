...
QueueHandle_t KLEE_xMutex;
TickType_t KLEE_xTicksToWait;
klee_make_symbolic_controllable(&KLEE_xMutex, sizeof(KLEE_xMutex), "KLEE_xMutex", true);
klee_make_symbolic_controllable(&KLEE_xTicksToWait, sizeof(KLEE_xTicksToWait), "KLEE_xTicksToWait", true);
if (KLEE_xMutex < KERNEL_REGION_START || KLEE_xMutex > KERNEL_REGION_END) { return 0; }
xQueueTakeMutexRecursive(KLEE_xMutex,KLEE_xTicksToWait);
return 0;
}
void vTaskFunction1( void *pvParameters )
{
