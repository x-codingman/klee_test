...
MemoryObject *Executor::lazyAllocTCBSymbolic(ExecutionState &state, size_t size,
  ...
  if (!mo) {
    ...
    bool needBound = false;
    ObjectPair op;
    ref<Expr> result = mo->getBaseExpr();
    if (state.addressSpace.lazyResolve(state, solver, result, op, needBound)) {
      ...
      terminateStateOnError(
          state,
          "Allocing memory object for struct.tskTaskControlBlock failed at "
          "lazyAllocTCBSymbolic",
          StateTerminationType::Ptr);
    }
    // bindLocal(target, state, mo->getBaseExpr());
  }
...
void Executor::executeMemoryOperation(
  ...
  //   //   for (ConstraintSet::constraints_ty::const_iterator
  //   i1=unbound->constraints.begin(),
  //   ie1=unbound->constraints.end();i1!=ie1;++i1){
  //   //     (*i1)->dump();
  //   //   }
  //   //   inBounds.get()->dump();
  //   // }
...
// add
void Executor::lazyMakeSymbolic(ExecutionState &state, const MemoryObject *mo,
                                const std::string &name) {
  unsigned id = 0;
